...
class PropagateLeg {
 public:
  PropagateLeg(const Eigen::Vector3d& v_mm_s_R,
               const Eigen::Vector3d& w_LR,
               double period_s)
      : v_mm_s_R_(v_mm_s_R),
        w_LR_(w_LR),
        pose_T2_T1_(
          Sophus::SO3d(
              Eigen::AngleAxisd(-period_s * w_LR.z(), Eigen::Vector3d::UnitZ())
              .toRotationMatrix()),
          -v_mm_s_R_ * period_s) {}
  ...
  Result operator()(const Eigen::Vector3d& position_mm_R) const {
    ...
    result.velocity_mm_s = -v_mm_s_R_ - w_LR_.cross(position_mm_R);
    return result;
  }

 private:
  Eigen::Vector3d v_mm_s_R_;
  Eigen::Vector3d w_LR_;
